...
<div
	...
	<!--<DashboardTopNav/>-->
	<div class="flex mt-6">
		<a href="./admin?tab=AuthorManager"
	...
	<div
		...
		<div class="w-full bg-white border rounded-md">
			<form action="./admin?tab=AuthorManager&action=${action}"
				method="POST">
				<c:if test="${action == 'update'}">
					<div class="font-bold mt-4 ml-4">Editing author #${authorUpdate.id}</div>
				</c:if>
				<div class="flex items-end">
					<div class="flex flex-col m-4">
						<label class="pb-2 font-bold">Author name</label> <input
							value="${authorUpdate.name}" type=text name="name"
							class="px-2 py-1 border rounded-md w-[500px]" minlength="1"
							maxlength="150" required />
							<input
							value="${authorUpdate.id}" type=hidden name="authorId" minlength="1"
							maxlength="150" required />
					</div>
					<button type="submit"
