...
<div class="">
    <div class="font-bold text-2xl">Tiểu thuyết</div>
    <div class="my-8">
        <%--<%@include file="SpecialItem.jsp" %>--%>
        <c:set var="specialItem" value="${tieuThuyet[0]}"/>
        <div class="border rounded-md p-8 flex gap-8 text-md">
            <c:if test="${specialItem.images.size() != 0}">
                <a href="./product?id=${specialItem.productId}">
                    <div style="background-image: url('${specialItem.images[0].url}')" class="aspect-[3/4] w-[160px] bg-center bg-cover bg-no-repeat rounded-md">
                    </div>
                </a>
            </c:if>
            <div class="w-full">
    ...
    <div class="grid grid-cols-5 gap-8 my-4">
        <c:forEach items="${tieuThuyet}" var="product" begin="1">
